...
while true; do
  ...
  case "$choice" in
    ...
    L|l)
      ...
      # Build the JSON payload content (escape quotes)
      # According to Google Gemini API docs: uses contents[] with parts[].text
      # We’ll create the JSON body string.
      json_body=$(jq -n --arg p "$user_prompt" '{contents:[{parts:[{text:$p}]}] }')
